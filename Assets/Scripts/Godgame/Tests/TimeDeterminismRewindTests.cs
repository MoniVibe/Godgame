...
{
    public class TimeDeterminismRewindTests
    {
        [Test]
        public void TimeDeterminismActor_RewindPlayback_RestoresStateDeterministically()
        {
            var world = new World("TimeDeterminismRewindTests");
            try
            {
                var entityManager = world.EntityManager;
                CoreSingletonBootstrapSystem.EnsureSingletons(entityManager);
                entityManager.CreateEntity(typeof(TimeDeterminismTag));

                var timeControlBootstrap = world.GetOrCreateSystem<TimeControlBootstrapSystem>();
                timeControlBootstrap.Update(world.Unmanaged);
                ...
                var bootstrap = world.GetOrCreateSystem<TimeDeterminismBootstrapSystem>();
                bootstrap.Update(world.Unmanaged);

                var motion = world.GetOrCreateSystem<TimeDeterminismMotionSystem>();
                var history = world.GetOrCreateSystem<TimeDeterminismHistorySystem>();
                ...
                var determinismEntity = entityManager.CreateEntityQuery(ComponentType.ReadOnly<TimeDeterminismState>()).GetSingletonEntity();
                var config = entityManager.GetComponentData<TimeDeterminismConfig>(determinismEntity);
                ...
                var preRewindState = entityManager.GetComponentData<TimeDeterminismState>(determinismEntity);
                ...
                var postRewindState = entityManager.GetComponentData<TimeDeterminismState>(determinismEntity);
                ...
                var resumedState = entityManager.GetComponentData<TimeDeterminismState>(determinismEntity);
