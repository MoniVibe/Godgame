...
namespace Godgame.Scenario
{
    ...
    public partial struct GodgameRewindGateHarnessSystem : ISystem
    {
        private static readonly FixedString64Bytes ScenarioId = new FixedString64Bytes("scenario.godgame.rewind_gate2");
        private static readonly FixedString64Bytes MetricHand = new FixedString64Bytes("gate2.godgame.actions.hand");
        private static readonly FixedString64Bytes MetricDelivery = new FixedString64Bytes("gate2.godgame.actions.delivery");
        private static readonly FixedString64Bytes MetricEntities = new FixedString64Bytes("gate2.godgame.entities.match");
        private static readonly FixedString64Bytes MetricTotals = new FixedString64Bytes("gate2.godgame.totals.match");
        private static readonly FixedString64Bytes MetricRewind = new FixedString64Bytes("gate2.godgame.rewind.match");
        private static readonly FixedString64Bytes WoodId = new FixedString64Bytes("wood");
        ...
        private int _baselineVillagers;
        private int _baselineResources;
        private int _baselineStorehouses;
        ...
        public void OnCreate(ref SystemState state)
        {
            ...
            _villagerQuery = state.GetEntityQuery(ComponentType.ReadOnly<Godgame.Villagers.VillagerId>());
            _resourceQuery = state.GetEntityQuery(ComponentType.ReadOnly<ResourceNodeTag>());
            _storehouseQuery = state.GetEntityQuery(ComponentType.ReadOnly<StorehouseConfig>());
        ...
        private void EnsureHandEntity(ref SystemState state)
        {
            ...
            state.EntityManager.AddComponentData(_handEntity, new GodgameGodHandTag());
            state.EntityManager.AddComponentData(_handEntity, new GodgameLegacyHandTag());
        ...
        private bool TryThrow(ref SystemState state)
        {
            ...
            var throwVelocity = new float3(4f, 6f, 2f);
        ...
        private bool TryDeliver(ref SystemState state, uint tick)
        {
            ...
            if (!state.EntityManager.HasComponent<StorehouseInventory>(storehouse))
            {
                state.EntityManager.AddComponentData(storehouse, new StorehouseInventory
                {
                    TotalStored = 0f,
                    TotalCapacity = 250f,
                    ItemTypeCount = 0,
            ...
            {
                if (!items[i].ResourceTypeId.Equals(WoodId))
                {
                    continue;
                }
            ...
            receipts.Add(new DeliveryReceipt
            {
                ...
                DeliveryTick = tick,
                ResourceTypeId = new FixedString32Bytes("wood")
            });
        ...
        {
            _baselineVillagers = _villagerQuery.CalculateEntityCount();
            _baselineResources = _resourceQuery.CalculateEntityCount();
            _baselineStorehouses = _storehouseQuery.CalculateEntityCount();
        ...
        private void EvaluateRewind(ref SystemState state)
        {
            ...
            var currentVillagers = _villagerQuery.CalculateEntityCount();
            var currentResources = _resourceQuery.CalculateEntityCount();
            var currentStorehouses = _storehouseQuery.CalculateEntityCount();
            var currentDeliveries = CountDeliveries(ref state);
            var currentInventory = SumStorehouseInventory(ref state);

            var entitiesMatch = currentVillagers == _baselineVillagers &&
                                currentResources == _baselineResources &&
                                currentStorehouses == _baselineStorehouses;
        ...
        {
            foreach (var (node, transform, entity) in SystemAPI.Query<RefRO<ResourceNodeTag>, RefRO<LocalTransform>>().WithEntityAccess())
            {
        ...
        private void LogFallbackOnce(Entity target, string missingComponents, bool skipped)
        {
            ...
            UnityEngine.Debug.LogWarning($"[GodgameRewindGateHarnessSystem] Missing {missingComponents} on entity {target.Index}:{target.Version}; {action}.");
        }
    }
}
