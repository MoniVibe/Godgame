...
{
    /// <summary>
    /// Handles throw queue release for Godgame: one-by-one or all-together via hotkeys.
    /// </summary>
    [UpdateInGroup(typeof(AfterPhysicsSystemGroup))]
    public partial struct GodgameThrowQueueSystem : ISystem
    {
        ...
        private void LogFallbackOnce(Entity target, string missingComponents, bool skipped)
        {
            ...
            UnityEngine.Debug.LogWarning($"[GodgameThrowQueueSystem] Missing {missingComponents} on entity {target.Index}:{target.Version}; {action}.");
        }
    }
}
