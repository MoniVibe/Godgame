...
{
    /// <summary>
    /// Records demo actor snapshots and restores them during rewind playback/catch-up.
    /// </summary>
    [BurstCompile]
    [UpdateInGroup(typeof(HistorySystemGroup))]
    public partial struct TimeDemoHistorySystem : ISystem
    {
        ...
        {
            var records = new NativeList<TimeDemoSnapshot>(Allocator.Temp);
            foreach (var (demoState, transform, entity) in SystemAPI
                         .Query<RefRO<TimeDemoState>, RefRO<LocalTransform>>()
                         .WithEntityAccess())
            {
                records.Add(new TimeDemoSnapshot
                {
                    Entity = entity,
                    State = demoState.ValueRO,
        ...
        private void Load(ref SystemState state, ref TimeStreamReader reader)
        {
            ...
            {
                var snapshot = reader.Read<TimeDemoSnapshot>();
                if (!SystemAPI.Exists(snapshot.Entity))
                {
                    continue;
                }

                if (SystemAPI.HasComponent<TimeDemoState>(snapshot.Entity))
                {
        ...
        private struct TimeDemoSnapshot
        {
            public Entity Entity;
            public TimeDemoState State;
