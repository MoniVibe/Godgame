        ...
        private sealed class Baker : Unity.Entities.Baker<GodgameSampleRegistryAuthoring>
        {
            ...
            private void BakeVillagers(GodgameSampleRegistryAuthoring authoring)
            {
                ...
                for (var i = 0; i < definitions.Length; i++)
                {
                    ...
                    // Add PureDOTS components (required for systems to work)
                    AddComponent(entity, new VillagerId
                    {
                        Value = definition.VillagerId,
                    ...
                    {
                        CurrentState = definition.AIState,
                        CurrentGoal = definition.AIGoal,
                        TargetEntity = Entity.Null
                    });

                    // Add GodgameVillager mirror component (for registry bridge)
            ...
            private void BakeStorehouses(GodgameSampleRegistryAuthoring authoring)
            {
                ...
                for (var i = 0; i < definitions.Length; i++)
                {
                    ...
                    // Add PureDOTS storehouse components (required for inventory management)
                    ...
                    if (resourceDefinitions != null)
                    {
                        for (var j = 0; j < resourceDefinitions.Length; j++)
                        {
                            ...
                            var resourceTypeIndex = resourceDefinition.ResourceTypeIndex;
                            
                            // Map resource type index to ResourceTypeId string
                            ...
                            {
                                resourceTypeId = new FixedString64Bytes("unknown");
                            }

                            // Add capacity element
                            if (resourceDefinition.Capacity > 0f)
                            {
                                capacityBuffer.Add(new StorehouseCapacityElement
                                {
                                    ResourceTypeId = resourceTypeId,
                                    MaxCapacity = math.max(0f, resourceDefinition.Capacity)
                                });
                            }

                            // Add inventory item if stored > 0
                            if (resourceDefinition.Stored > 0f)
                            {
                                inventoryBuffer.Add(new StorehouseInventoryItem
                                {
                                    ResourceTypeId = resourceTypeId,
                                    Amount = math.max(0f, resourceDefinition.Stored),
