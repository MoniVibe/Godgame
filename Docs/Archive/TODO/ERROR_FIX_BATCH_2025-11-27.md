...
**Status**: Resolved by adding a `Godgame.Presentation` runtime asmdef (Assets/Scripts/Godgame/Presentation) and pointing `Godgame.Presentation.Editor` to it so `SwappablePresentationBindingEditor` can see `PresentationRegistry`/`SwappablePresentationBindingAuthoring` in `Godgame.Presentation`.
